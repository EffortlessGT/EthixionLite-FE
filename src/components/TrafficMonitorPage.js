...
import { getCurrentUser, getReportLogs, getDashboardSecurityDetails, getDashboardTrendsDetails, getDashboardTrendsStatusDetails, getDashboardAdvanceMonitorsDetails } from '../api';
        ...
        const fetchSecurityData = async () => {
            try {
                ...
                if (result && result.securitydata) {
                    const cleanedTopThreats = result.securitydata.top_threat_types.map(threat => ({
                        threat: threat.threat || "Unknown Threat",
                        count: Number(threat.count) || 0,
                    }));

                    const cleanedThreatsOverTime = result.securitydata.threats_over_time.map(entry => ({
                        ...entry,
        ...
        const fetchTrendsData = async () => {
            try {
                ...
                if (result && result.trenddata) {
                    setUserActivity(result.trenddata.user_activity.map(entry => ({
                        ...entry,
                        count: Number(entry.count)
                    })));

                    setUserAgents(result.trenddata.user_agents.map(entry => ({
                        ...entry,
                        count: Number(entry.count)
                    })));

                    setPeakTraffic(result.trenddata.peak_traffic.map(entry => ({
                        ...entry,
                        hour: new Date(entry.hour).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                        count: Number(entry.count)
                    })));

                    setWeekdayTraffic(result.trenddata.weekday_traffic.map(entry => ({
                        day_of_week: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][entry.day_of_week],
                        count: Number(entry.count)
                    })));

                    setGeoDistribution(
                        result.trenddata.geo_distribution.map(entry => ({
                            ip_address: entry.ip_address ? String(entry.ip_address) : "Unknown",
                            count: Number(entry.count)
                        }))
                    );
        ...
        const fetchTrendsStatusData = async () => {
            try {
                ...
                if (result && result.trends_status) {
                    setStatusPieData(result.trends_status.status_pie.map(item => ({
                        name: item.status ? 'Success' : 'Failure',
                        value: Number(item.count)
                    })));

                    setApiStatusBreakdown(result.trends_status.api_status_breakdown.map(item => ({
                        apiname: item.apiname,
                        success: Number(item.success),
                        failure: Number(item.failure)
                    })));

                    setFailuresOverTime(result.trends_status.failure_timeline.map(entry => ({
                        ...entry,
                        date: new Date(entry.date).toLocaleDateString(),
        ...
        const fetchAdvanceMonitorsData = async () => {
            try {
                ...
                if (result && result.adv_monitors) {
                    setApiHeatmapData(result.adv_monitors.api_threat_heatmap.map(entry => ({
                    ...
                    setIpReliability(result.adv_monitors.success_rate_data.map(entry => {
                        const total = Number(entry.success || 0) + Number(entry.failure || 0);
                    ...
                    setAutoFlaggedIPs(result.adv_monitors.flagged_abuse.map(entry => ({
                        ip: entry.ip_address,
                        failures: Number(entry.failure || 0)
                    })));

                    setRateAbusePatterns(result.adv_monitors.rate_limit_alerts.map(entry => ({
                        ip: entry.ip_address,
    ...
    const volumeContainer = error ? (
        ...
        <>
            ...
            <div className="monitor-grid">
                <div className="monitor-card"><h3>Total Requests</h3><p>{totalRQ} Today</p></div>
                <div className="monitor-card"><h3>Allowed Requests</h3><p>{allowedRQ} Today</p></div>
                <div className="monitor-card"><h3>Blocked Requests</h3><p>{blockedRQ} Today</p></div>
                <div className="monitor-card"><h3>Suspicious Requests</h3><p>{suspiciousRQ} Today</p></div>
            </div>
        </>
    );
                    ...
                    {apiHeatmapData && apiHeatmapData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={apiHeatmapData}>
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="threat_count" fill="#f39c12" name="Threat Count" barSize={40} minPointSize={10} />
                                <Bar dataKey="total_requests" fill="#3498db" name="Total Requests" barSize={40} minPointSize={10} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
                    ...
                    {ipReliability && ipReliability.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={ipReliability}>
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="success" fill="#2ecc71" barSize={40} minPointSize={10} />
                                <Bar dataKey="failure" fill="#e74c3c" barSize={40} minPointSize={10} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
                        <p>No data available</p>
                    ...
                    {autoFlaggedIPs && autoFlaggedIPs.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={autoFlaggedIPs}>
                                ...
                                <YAxis />
                                <Tooltip />
                                <Bar dataKey="failures" fill="#e74c3c" barSize={40} minPointSize={10} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
                        <p>No flagged IPs</p>
                    ...
                    {rateAbusePatterns && rateAbusePatterns.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={rateAbusePatterns}>
                                ...
                                <YAxis />
                                <Tooltip />
                                <Bar dataKey="frequency" fill="#9b59b6" barSize={40} minPointSize={10} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
                ...
                <div className={`dashboard-container ${menuOpen ? 'sidebar-open' : ''}`}>
                    ...
                    <div className="dash-dataContainer">
                        <div className="monitor-tabs">
                            <button onClick={() => setCurrentView("volume")} className={currentView === "volume" ? "active" : ""}>📊 Volume</button>
                            <button onClick={() => setCurrentView("security")} className={currentView === "security" ? "active" : ""}>🔐 Security</button>
                            <button onClick={() => setCurrentView("trends")} className={currentView === "trends" ? "active" : ""}>🗓️ Trends</button>
                            <button onClick={() => setCurrentView("advance_monitors")} className={currentView === "advance_monitors" ? "active" : ""}>🌍 Advance Monitorings</button>
                            <button onClick={() => setCurrentView("status")} className={currentView === "status" ? "active" : ""}>⚙️ Status</button>
                        </div>
