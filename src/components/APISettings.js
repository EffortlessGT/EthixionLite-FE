          ...
          {showRegenerated ? (
              ...
              <div className="regeratedAPIDIV">
                {regStatus ? (
                  <>
                    ...
                    <strong>New API key Generated for {myapiname}.</strong>
                    <br />
                    <strong>Your API Key: {key}</strong>
                    <p>
                      Your new Ethixion API key has been created. Keep it
                      private and never share it publicly. Use it only in
                      authorized applications.
                    </p>
                    <button onClick={() => setShowRegenerated(false)}>
                      I Understand
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    <p>
                      The Ethixion server encountered an error while processing
                      your request to regenerate the API key for{' '}
                      <b>{myapiname}</b>. Please try again later or contact
            ...
            <div className="dash-dataContainer" id="dash-dataContainer">
              ...
              <div className="api-settings-nav">
                {['Overview', 'API Keys', 'Integration', 'Alerts & Logs'].map(
                  (tab) => (
                    <button
                      key={tab}
              ...
              <div className="api-settings-content">
                        ...
                        {apiData && apiData.apis && apiData.apis.length > 0 ? (
                          apiData.apis.map((api, idx) => (
                            <tr key={idx}>
                              ...
                              <td>
                                {api.createdon
                                  ? new Date(api.createdon).toLocaleString()
                                  : 'unknown'}
                              </td>
                              <td>
                                {api.last_used
                              ...
                              <td>
                                {api.threats_blocked &&
                                api.threats_blocked.length > 0
                                  ? api.threats_blocked.join(', ')
                                  : 'None'}
                              </td>
                            </tr>
                          ))
                          ...
                          <tr>
                            <td colSpan="7">
                              No API data available. Please try again later.
                            </td>
                          </tr>
                        )}
                      </tbody>
                        ...
                        {apiData && apiData.apis && apiData.apis.length > 0 ? (
                          apiData.apis.map((api, idx) => (
                            <tr key={idx}>
                              ...
                              <td>
                                {api.active_status ? 'Active' : 'Inactive'}
                              </td>
                              <td>
                                {api.active_status ? (
                                  <form
                ...
                {activeApiTab === 'Alerts & Logs' && (
                  <div>
                    <h2>📊 Alerts & Logs</h2>
                    <p>
                      View API activity logs, monitor blocked threats, and
                      analyze traffic insights.
                    </p>
                    <table>
                      <thead>
                        <tr>
