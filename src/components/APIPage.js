...
function APIPage() {
  ...
  const handleAPICreation = async (e) => {
    ...
    const allowedIpPatt = /^\s*$|^(\d{1,3}\.){3}\d{1,3}(\s*,\s*(\d{1,3}\.){3}\d{1,3})*$/;
    ...
    try {
      ...
      if (result.apiname != null && result.apikey) {
        document.getElementById('apiid').innerText = 'Your API ID: ' + result.apiname;
        document.getElementById('apikey').innerText = 'Your API Key: ' + result.apikey;
      ...
      <main>
        <div className='api-IdeaContainer'>
          ...
          <div className='desccontainer'>
            ...
            <p>The Ethixion API lets you integrate Ethixion’s Application Layer Firewall directly into your web applications. It analyzes incoming traffic, detects malicious behavior (like SQL injection, XSS, etc.), and helps block harmful requests before they reach your server. Incorporating the Ethixion API into your infrastructure allows you to enforce custom security rules, monitor traffic behavior, and receive detailed threat intelligence — all through a simple and developer-friendly interface.</p>
            <span>⚠️ Why is it Important?</span>
            <ul>
              <li>🛡️ Protects your application from common web attacks</li>
              <li>🚀 Easy to integrate with your backend using simple API calls</li>
              <li>📊 Provides real-time threat detection and logging</li>
              <li>⚙️ Adds an extra security layer without changing your application code</li>
            </ul>
          </div>
        </div>
        <br />
        <div className='api'>
          ...
          <p>Define your API's basic information and secure it to protect your application.</p>

          <div className='apisteps-container'>
            <div className='box'>
              <span>Step 1</span>
              <h3>API Info</h3>
              <p>Enter basic details like the name and description of your API.</p>
            </div>
            <div className='box'>
              <span>Step 2</span>
              <h3>Set Access Rules</h3>
              <p>Define how your API can be accessed for example, by IP restrictions or enabling authentication.</p>
            </div>
            <div className='box'>
              <span>Step 3</span>
              <h3>Generate API Key</h3>
              <p>Generate your API key to start integrating Ethixion Firewall into your application.</p>
            </div>
          </div>
        </div>
        ...
        <div className='api-form'>
          <form method='POST' onSubmit={handleAPICreation}>
            <div className='apiform-boxes'>
              ...
              <input type='text' id='apiName' name='apiName' value={apiname} onChange={(e) => setApiname(e.target.value)} placeholder='Enter API Name' />
            </div>

            <div className='apiform-boxes'>
              <label htmlFor='apiDescription'>API Description:</label>
              <textarea id='apiDescription' name='apiDescription' value={apidesc} onChange={(e) => setApidesc(e.target.value)} placeholder='Enter API description within 100 characters.'></textarea>
            </div>

            <div className='apiform-boxes'>
              <label htmlFor='endpointUrl'>Endpoint URL:</label>
              <input type='url' id='endpointUrl' name='endpointUrl' value={endpointUrl} onChange={(e) => setEndpointUrl(e.target.value)} placeholder='Enter base endpoint URL' />
            </div>

            <div className='apiform-boxes'>
              <label htmlFor='authType'>Authentication Type:</label>
              <select id='authType' name='authType' value={authType} onChange={(e) => setAuthType(e.target.value)}>
            ...
            <div className='apiform-boxes'>
              ...
              <input type='text' id='allowedIps' name='allowedIps' value={allowedIp} onChange={(e) => setAllowedIp(e.target.value)} placeholder='E.g., 192.168.1.1, 127.0.0.1' />
            </div>
            ...
            <div className='apiform-boxes'>
              ...
              >
                ...
                <option value='50'>50 req/min – Maximum (production-ready)</option>
              </select>
              <p className="hint">
                Choose the right request limit depending on your API usage (sandbox, hobby, or business app).
              </p>
            </div>
            ...
            <div className='apiform-boxes'>
              ...
              <div className='filterscontainer'>
                <label>
                  <input type='checkbox' name='filters' value='sqlInjection' checked={threatFilters.includes('sqlInjection')} onChange={handleFilterChange} />
                  SQL Injection
                </label><br />

                <label>
                  <input type='checkbox' name='filters' value='xss' checked={threatFilters.includes('xss')} onChange={handleFilterChange} />
                  Cross-Site Scripting (XSS)
                </label><br />

                <label>
                  <input type='checkbox' name='filters' value='userAgent' checked={threatFilters.includes('userAgent')} onChange={handleFilterChange} />
        ...
        <div className='APICredentials'>
          ...
          <p>Please keep your API credentials confidential and do not share them with unauthorized parties.</p>
