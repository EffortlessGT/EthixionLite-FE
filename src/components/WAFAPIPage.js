  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!result || !result.wafid || !result.wafkey) {
        toast.error('Failure Occurred: ' + (result?.message || 'Unknown error'));
        return;
      }
      ...
      toast.error('Failure Occurred: ' + (error.message || 'Unknown network/server error'));
    }
      ...
      <main>
        <div className="waf-intro">
          ...
          <div className="desccontainer">
            ...
            <p>
              The Ethixion Web Application Firewall (WAF) acts as a smart security gateway between your users and your application servers. Every HTTP request first passes through this intelligent protection layer, where it is deeply inspected for anomalies, threats, and malicious intent. Using advanced pattern recognition and behavioral analysis, Ethixion WAF can detect and block attacks such as SQL Injection, Cross-Site Scripting (XSS), Remote Code Execution (RCE), CSRF, file upload exploits, and even large-scale DDoS floods before they ever touch your infrastructure. Beyond just blocking, Ethixion continuously monitors traffic in real time, learns from request patterns, and adapts dynamically to new attack techniques.
            </p>
        ...
        <FadeUpOnScroll >
          <div className="waf-steps">
            ...
            <div className="steps-container">
              <div className="box"><span>Step 1</span><h3>Setup</h3><p>Enter WAF name, description, and your domain.</p></div>
              <div className="box"><span>Step 2</span><h3>Configure Rules</h3><p>Select your inspection mode, rules, and rate limits.</p></div>
              <div className="box"><span>Step 3</span><h3>Generate Keys</h3><p>Deploy your secure firewall API and get credentials.</p></div>
            </div>
          </div>
        </FadeUpOnScroll>
        <FadeUpOnScroll>
          ...
          <div className="waf-form">
            <form method="POST" onSubmit={handleSubmit}>
              <div className="form-box">
                ...
                <input type="text" value={wafName} onChange={(e) => setWafName(e.target.value)} placeholder="MySite-WAF" required />
              </div>
              <div className="form-box">
                <label>Description:</label>
                <textarea value={wafDesc} onChange={(e) => setWafDesc(e.target.value)} placeholder="Short description of your WAF purpose" />
              </div>
              <div className="form-box">
                <label>Protected Domain / Endpoint:</label>
                <input type="text" value={protectedDomain} onChange={(e) => setProtectedDomain(e.target.value)} placeholder="https://example.com" required />
              </div>
              <div className="form-box">
                ...
                <div className="checkboxes">
                  {['GET', 'POST', 'PUT', 'DELETE','PATCH'].map((method) => (
                    <label key={method}>
                      <input type="checkbox" value={method} checked={httpallowedmethods.includes(method)} onChange={(e) => {
                        if (e.target.checked) {
                          setHttpAllowedMethods([...httpallowedmethods, method]);
                        } else {
                          setHttpAllowedMethods(httpallowedmethods.filter(m => m !== method));
                        }
                      }} />
                      {method}
                    </label>
              ...
              <div className="form-box">
                ...
                <input type="text" value={proxyDomain} onChange={(e) => setProxyDomain(e.target.value)} placeholder="https://example.com" required />
              </div>

              <div className="form-box">
                <label>Inspection Mode:</label>
                <select value={inspectionMode} onChange={(e) => setInspectionMode(e.target.value)} required>
              ...
              <div className="form-box waf-rules">
                ...
                <div className="filterscontainer">
                  {['SQL Injection', 'Cross Site Scripting', 'File Upload', 'RCE', 'CSRF', 'Bot Detection'].map((rule, i) => (
                    <label key={i}>
                      <input type="checkbox" value={rule.toLowerCase().replace(/\s/g, '')} onChange={handleThreatChange} checked={threatRules.includes(rule.toLowerCase().replace(/\s/g, ''))} />
                      {rule}
                    </label>
                  ))}
                </div>
              ...
              <div className="form-box">
                ...
                <input type="number" min="1" max="100" value={rateLimit} onChange={(e) => setRateLimit(e.target.value)} placeholder="e.g., 100" />
              </div>
              ...
              <div className="form-box">
                ...
                <select value={logging} onChange={(e) => setLogging(e.target.value)}>
              ...
              <div className="form-box">
                ...
                <input type="text" value={geoBlock} onChange={(e) => setGeoBlock(e.target.value)} placeholder="e.g., CN, RU, IR" />
              </div>
        ...
        <div className="WAFCredentials">
          ...
          <p>Use these credentials in your reverse proxy or app configuration to activate protection.</p>
